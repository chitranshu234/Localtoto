    ...
    reverseGeocode: async (lat: number, lng: number): Promise<ReverseGeocodeResult> => {
        try {
            ...
            if (data.place_name) {
                ...
                return { address: data.address, display_name: data.address };
            ...
            return { address: 'Your current location', display_name: 'Your current location' };

        } catch (error) {
            console.log('Reverse geocode error (non-critical):', error);
            return {
